...
def load_csv_files():
    x = []
    y = []
    datafile_ham = "/home/hustlib/2book/data/ai_sms/ham_data.csv"
    datafile_spam = "/home/hustlib/2book/data/ai_sms/spam_data.csv"
    print("load file:", datafile_ham)


    data_df_ham = pd.read_csv(
        datafile_ham,
        sep=',',
        header=0
    )

    for index, line in data_df_ham.iterrows():
        label = line['label']
    ...
    data_df_spam = pd.read_csv(
        datafile_spam,
        sep=',',
        header=0
    )

    for index, line in data_df_spam.iterrows():
        label = line['label']
        msg = line['message']

        msg_list = jieba.cut(msg, cut_all=False)
...
def load_csv_files_with_jieba():
    print("load files with jieba")
    x = []
    y = []
    datafile_ham = "/home/hustlib/2book/data/ai_sms/ham_data.csv"
    datafile_spam = "/home/hustlib/2book/data/ai_sms/spam_data.csv"
    ...
    for index, line in data_df_ham.iterrows():
        ...
        msg_list = jieba.cut_for_search(msg)
        message = " ".join(msg_list)
...
def load_csv_files_02():
    x = []
    y = []
    datafile = "/home/hustlib/2book/data/ai_sms/spam_data_02.csv"
    print("load file:", datafile)

    data_df_email = pd.read_csv(
        datafile,
        sep=',',
        header=0
    )

    for index, line in data_df_email.iterrows():
        label = line['v1']
        message = line['v2']
        x.append(message)
        if label == 'ham':
...
def get_stopwords_cn():
    path = "/home/hustlib/2book/data/ai_sms/scu_stopwords.txt"
...
def get_stopwords_en():
    path = "/home/hustlib/2book/data/ai_sms/english.txt"
    with open(path, 'r') as f:
        stopwords = f.read().decode('utf-8')
    stopwords = stopwords.splitlines()
    return stopwords

def load_csv_files_cn(num):
    x = []
    y = []
    datafile = "/home/hustlib/2book/data/ai_sms/sms_pub.csv"
    print("load file:", datafile)
    data_df_email = pd.read_csv(
        datafile,
        sep=',',
    ...
    stopwords = get_stopwords_cn()
    counter = 0
    for index, line in data_df_email.iterrows():
        label = line['label']
        msg = line['message']
        message = jieba.cut(msg, cut_all=False)
        msg_list = []
        for keyword in message:
            if keyword in stopwords:
                continue
            else:
                msg_list.append(keyword)
...
def load_all_files(num):
    x = []
    y = []
    counter = 0
    datafile = "/home/hustlib/2book/data/sms/smsspamcollection/SMSSpamCollection.txt"
    # datafile = "/home/hustlib/2book/data/ai_sms/sms_pub.csv"
...
def do_cnn_word2vec_2d_345(trainX, testX, trainY, testY):
    ...
    network = merge([branch1, branch2, branch3], mode='concat', axis=1)
    network = tf.expand_dims(network, 2)
    network = max_pool_2d(network)#global_max_pool_2d(network)
    network = dropout(network, 0.8)
    network = fully_connected(network, 2, activation='softmax')
    network = regression(network, optimizer='adam', learning_rate=0.001,
                         loss='categorical_crossentropy', name='target')
...
def do_cnn_word2vec_02(trainX, testX, trainY, testY):
    global max_features
    print "CNN and word2vec"
    y_test = testY

    print "Original trainX shape: {}".format(trainX.shape)
    print "Original testX shape: {}".format(testX.shape)

    if trainX.shape[1] != max_features:
        trainX = pad_sequences(trainX, maxlen=max_features, value=0.)
        testX = pad_sequences(testX, maxlen=max_features, value=0.)
...
def handle_precision(y_test, y_predict_list):
    y_predict = []
    for i in y_predict_list:
        print  i[0]
        if i[0] > 0.5:
...
def do_rnn_word2vec(trainX, testX, trainY, testY):
    ...
    handle_precision(y_test, y_predict_list)
...
def xgb_model_predict(x_test, y_test):
    xgb_model = xgb.Booster()
    xgb_model.load_model('xgb_model')
    dtest = xgb.DMatrix(x_test)
    y_pred_proba = xgb_model.predict(dtest)
...
def  get_features_by_tf():
    global  max_document_length
    x_train, x_test, y_train, y_test=load_all_files(3000)
    # x_train, x_test, y_train, y_test=load_csv_files_cn(3000)
    # x_train, x_test, y_train, y_test = load_csv_files_02()
...
def  get_features_by_tf_02():
    global  max_document_length
    # x_train, x_test, y_train, y_test=load_all_files()
    # x_train, x_test, y_train, y_test=load_csv_files()
    x_train, x_test, y_train, y_test = load_csv_files_02()
    vp=tflearn.data_utils.VocabularyProcessor(max_document_length=max_document_length,
                                              min_frequency=0,
                                              vocabulary=None,
...
def  get_features_by_tf_03(num=5000):
    global  max_document_length
    # x_train, x_test, y_train, y_test=load_all_files()
    # x_train, x_test, y_train, y_test=load_csv_files()
    x_train, x_test, y_train, y_test = load_csv_files_03(num)
    vp=tflearn.data_utils.VocabularyProcessor(max_document_length=max_document_length,
                                              min_frequency=0,
                                              vocabulary=None,
                                              tokenizer_fn=None)
...
def  get_features_by_word2vec(num=5000):
    global  max_features
    global word2ver_bin
    x_train, x_test, y_train, y_test=load_csv_files_03(num)
    # x_train, x_test, y_train, y_test = load_csv_files_with_jieba()
...
def get_data():
    datafile = "/home/hustlib/2book/data/ai_sms/sms_pub.csv"
    datafile2 = "/home/hustlib/2book/data/ai_sms/email_data.csv"
    df = pd.read_csv(datafile, nrows=15000)
    df.to_csv(datafile2, index=False)
...
if __name__ == "__main__":
    ...
    # x_train, x_test, y_train, y_test = get_features_by_tf_02()
    # x_train, x_test, y_train, y_test = get_features_by_tf_03(50000)
    ...
    # x_train, x_test, y_train, y_test = get_features_by_word2vec(200)
    ...
    # xgb_model_train(x_train, x_test, y_train, y_test)
    # xgb_model_predict(x_test, y_test)
    ...
    #CNN
    # do_cnn_word2vec_02(x_train, x_test, y_train, y_test)
    ...
    # best test_size = 0.3
    # rf
    # do_rf_doc2vec(x_train, x_test, y_train, y_test)           # 0.5
    # do_rf_word2vec(x_train, x_test, y_train, y_test)          # 00
    # do_xgboost_word2vec(x_train, x_test, y_train, y_test)     # 00
    # do_xgboost_wordbag(x_train, x_test, y_train, y_test)      # 00
    #
    #RNN
    # do_rnn_word2vec(x_train, x_test, y_train, y_test)     # save model
