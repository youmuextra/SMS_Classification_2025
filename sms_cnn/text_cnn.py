...
class TextCNN(object):
    ...
    def __init__(self, text_len, num_classes, embedding_size,
        ...
        #输入，输出，dropout占位符
        self.input_x = tf.compat.v1.placeholder(tf.float32, [None, text_len, embedding_size], name="input_x")
        ...
        with tf.name_scope("output"):
            W = tf.compat.v1.get_variable("W", shape=[num_convs_total, num_classes],
                                initializer = tf.keras.initializers.GlorotNormal())
            b = tf.Variable(tf.constant(0.1, shape=[num_classes], name="b"))
            l2_loss += tf.nn.l2_loss(W)
            l2_loss += tf.nn.l2_loss(b)
            self.scores = tf.matmul(self.h_drop, W) + b
            self.scores = tf.identity(self.scores, name="scores")
